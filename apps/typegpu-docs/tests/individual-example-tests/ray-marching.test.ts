  ...
  it('should produce valid code', async ({ device }) => {
    ...
    expect(shaderCodes).toMatchInlineSnapshot(`
      ...
      fn getMorphingShape(p: vec3f, t: f32) -> Shape {
        ...
        let sphere2Offset = vec3f((cos(((t * 2f) + 3.14f)) * 0.8f), (sin(((t * 3f) + 1.57f)) * 0.3f), (sin(((t * 2f) + 3.14f)) * 0.8f));
        let sphere1 = Shape(vec3f(0.4000000059604645, 0.5, 1), sdSphere((localP - sphere1Offset), 0.5f));
        let sphere2 = Shape(vec3f(1, 0.800000011920929, 0.20000000298023224), sdSphere((localP - sphere2Offset), 0.3f));
        let box = Shape(vec3f(1, 0.30000001192092896, 0.30000001192092896), sdBoxFrame3d(rotatedP, boxSize, 0.1f));
        let spheres = smoothShapeUnion(sphere1, sphere2, 0.1f);
      ...
      fn getSceneDist(p: vec3f) -> Shape {
        let shape = getMorphingShape(p, time);
        let floor_1 = Shape(mix(vec3f(1), vec3f(0.20000000298023224), checkerBoard((p.xz * 2f))), sdPlane(p, vec3f(0, 1, 0), 0f));
      ...
      fn getNormal(p: vec3f) -> vec3f {
        ...
        const e = 0.01;
        let n = vec3f((getSceneDist((p + vec3f(e, 0f, 0f))).dist - dist), (getSceneDist((p + vec3f(0f, e, 0f))).dist - dist), (getSceneDist((p + vec3f(0f, 0f, e))).dist - dist));
        return normalize(n);
      }
      ...
      @fragment fn fragmentMain(_arg_0: fragmentMain_Input) -> @location(0) vec4f {
        ...
        let n = getNormal(p);
        let lightPos = getOrbitingLightPos(time);
        ...
        let litColor = (march.color * diff);
        let finalColor = mix((litColor * 0.5f), litColor, shadow);
