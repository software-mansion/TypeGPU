  ...
  it('should produce valid code', async ({ device }) => {
    ...
    expect(shaderCodes).toMatchInlineSnapshot(`
      ...
      @fragment fn fragmentShader(_arg_0: fragmentShader_Input) -> @location(0) vec4f {
        ...
        let destructured = (&exampleControlsUniform);
        let ambientColor = (&(*destructured).ambientColor);
        let ambientStrength = (*destructured).ambientStrength;
        let specularExponent = (*destructured).specularExponent;
