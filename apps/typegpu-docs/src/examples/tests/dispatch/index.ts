...
async function test2d(): Promise<boolean> {
  ...
  const mutable = root.createMutable(d.arrayOf(d.arrayOf(d.vec2u, size[1]), size[0]));
  root
...
async function testWorkgroupSize(): Promise<boolean> {
  ...
  root
    .createGuardedComputePipeline((_x, _y, _z) => {
      'use gpu';
...
async function testMultipleDispatches(): Promise<boolean> {
  const size = [7] as const;
  const mutable = root.createMutable(d.arrayOf(d.u32, size[0]), [0, 1, 2, 3, 4, 5, 6]);
...
async function testDifferentBindGroups(): Promise<boolean> {
  ...
  const buffer1 = root.createBuffer(d.arrayOf(d.u32, 3), [1, 2, 3]).$usage('storage');
  const buffer2 = root.createBuffer(d.arrayOf(d.u32, 4), [2, 4, 8, 16]).$usage('storage');
...
async function testSlots(): Promise<boolean> {
  ...
  root.with(valueSlot, 3).createGuardedComputePipeline(main).dispatchThreads(); // add 3
