...
const handleDraw = root.createGuardedComputePipeline((x, y) => {
  ...
  randf.seed2((d.vec2f(x, y) / d.f32(gameSizeUniform.$)) * timeUniform.$);
  if (sdLine(uv, stroke.start, stroke.end) <= getBrushRadius(stroke.radius, gameSizeAccessor.$)) {
    let out = d.u32(0);
    ...
    std.textureStore(computeLayout.$.next, d.vec2u(x, y), d.vec4u(out, 0, 0, 0));
  }
});
...
const handleDrawBitpacked = root.createGuardedComputePipeline((px, py) => {
  ...
  const current = std.textureLoad(computeLayout.$.current, d.vec2u(px, py), d.i32(0)).x;
  ...
  for (let i = 0; i < 32; i++) {
    ...
    if (sdLine(uv, stroke.start, stroke.end) <= getBrushRadius(stroke.radius, gs)) {
  ...
  const result = (current & (affectedMask ^ d.u32(0xffffffff))) | (newValues & affectedMask);

  std.textureStore(computeLayout.$.next, d.vec2u(px, py), d.vec4u(result, 0, 0, 0));
});

const perf = { sum: 0, count: 0, window: 120 };

function createPipeline(name: string, compute: TgpuComputeFn) {
  const base = root.with(gameSizeAccessor, gameSizeUniform).createComputePipeline({ compute });

  return !hasTimestamp
    ? base
    : base.withPerformanceCallback((start, end) => {
        perf.sum += Number(end - start) / 1_000_00;
        perf.count++;
        if (perf.count >= perf.window) {
          console.log(
            `${name}: ${(perf.sum / perf.count).toFixed(3)}ms average over ${perf.window} frames`,
          );
          perf.sum = 0;
          perf.count = 0;
        }
...
const sampleRegular = (sampleUv: d.v2f, gs: number): number => {
  'use gpu';
  return std.textureLoad(displayLayout.$.source, d.vec2u(sampleUv * gs)).x;
};
...
const displayFragment = tgpu.fragmentFn({
  ...
  if (inMinimap) {
    ...
    const dist = sdRoundedBox2d(localUv - clampedCenter, d.vec2f(viewSize / 2), 0.01);
  ...
  const alive = std.select(
    std.normalize(d.vec4f(sampleUv.x / 1.5, sampleUv.y / 1.5, 1 - sampleUv.x / 1.5, 1)),
    d.vec4f(1),
    isClassic,
  );
...
const [displayPipeline, bitpackedDisplayPipeline] = [sampleRegular, sampleBitpacked].map((fn) =>
  root.with(cellSamplerSlot, fn).createRenderPipeline({
...
const bitpackedRandomInit = root.createGuardedComputePipeline((x, y) => {
  ...
  let packed = d.u32(0);
  for (let i = 0; i < 32; i++) {
    randf.seed2((d.vec2f(d.f32(x) * 32 + d.f32(i), y) / d.f32(gameSizeUniform.$)) * timeUniform.$);
    packed = packed | (std.select(d.u32(0), d.u32(1), randf.sample() > 0.5) << d.u32(i));
  }
  std.textureStore(computeLayout.$.next, d.vec2u(x, y), d.vec4u(packed, 0, 0, 0));
});
...
const stepOnce = (timestamp: number) => {
  ...
  computePipelines[chosenPipeline]
    ...
    .dispatchWorkgroups(
      Math.max(1, gameSize / (chosenPipeline === 'bitpacked' ? TILE_SIZE * 32 : TILE_SIZE)),
      gameSize / TILE_SIZE,
    );
};
...
function frame(timestamp: number) {
  ...
  if (input.drawPos) {
    ...
    pipeline.with(computeBindGroups[even]).dispatchThreads(x, y);
