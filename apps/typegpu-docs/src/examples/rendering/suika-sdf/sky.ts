...
export const bucketBg = (scenePos: d.v2f, daylight: number) => {
  ...
  const color = std.mix(d.vec3f(0.07, 0.07, 0.13), d.vec3f(0.92, 0.89, 0.84), daylight);
...
export const skyColor = (uv: d.v2f, daylight: number, time: number) => {
  ...
  const daySky = std.mix(d.vec3f(0.98, 0.93, 0.78), d.vec3f(0.62, 0.86, 1.0), skyMix);
  const nightSky = std.mix(d.vec3f(0.05, 0.04, 0.12), d.vec3f(0.02, 0.02, 0.1), skyMix);
  ...
  bgColor += d.vec3f(
    starDot * std.step(0.9, starExists) * starBright * (1 - daylight) * twinkle * 2.5,
  );
  ...
  const sunMask = std.smoothstep(0.09, 0.055, sunDist) * std.clamp(sinT * 6, 0, 1);
  ...
  const moonMask = std.smoothstep(0.065, 0.04, moonDist) * std.clamp(-sinT * 5, 0, 1);
  bgColor = std.mix(bgColor, d.vec3f(0.93, 0.95, 1.0), moonMask);

  // Dawn/dusk glow along the horizon
  const dawnDusk = std.smoothstep(-0.2, 0.12, sinT) * std.smoothstep(0.55, 0.12, sinT);
  const horizonBand = std.clamp(0.32 - std.abs(uv.y - 0.77), 0, 1) * 3.2;
  bgColor = std.mix(bgColor, d.vec3f(0.95, 0.45, 0.12), dawnDusk * horizonBand * 0.65);

  // Hills
  const hillX = uv.x + time * 0.025;
  const hillLine = 0.8 + std.sin(hillX * 4.5) * 0.07 + std.sin(hillX * 1.8) * 0.04;
  const hillMask = std.clamp((uv.y - hillLine) * 50, 0, 1);
  const hillColor = std.mix(d.vec3f(0.1, 0.2, 0.12), d.vec3f(0.41, 0.76, 0.46), daylight);
