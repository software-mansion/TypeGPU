...
import { BlockInstance, type Chunk } from './schemas.ts';
...
export function generate(chunkIndex: d.v3i): Chunk {
    ...
    for (let y = 0; y < CHUNK_SIZE; y++) {
      for (let z = 0; z < CHUNK_SIZE; z++) {
        ...
        const result = tgpu['~unstable'].simulate(() => {
          ...
          return perlin3d.sample(sampleIndex * 0.2) + y / 16;
        }).value;
...
export function chunkToInstanceData(chunk: Chunk) {
  const instanceData: d.Infer<typeof BlockInstance>[] = [];
