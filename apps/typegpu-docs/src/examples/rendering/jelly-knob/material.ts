...
export function bottomReflectionMask(position: d.v3f, normal: d.v3f, camera: d.v3f) {
  'use gpu';
  // Restrict the dark reflection to the low, downward-curving edge facing the camera.
  const towardCamera = std.normalize(camera.xz - position.xz);
...
export function jellyReflection(position: d.v3f, normal: d.v3f, camera: d.v3f) {
  'use gpu';
