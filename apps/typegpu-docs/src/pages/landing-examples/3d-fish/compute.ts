...
export const simulate = (fishIndex: number) => {
  ...
  if (cohesionCount > 0) {
    cohesion = std.sub(std.mul(1 / d.f32(cohesionCount), cohesion), fishData.position);
  }
  for (let i = 0; i < 3; i += 1) {
  ...
  if (layout.$.mouseRay.activated === 1) {
    const proj = projectPointOnLine(fishData.position, layout.$.mouseRay.line);
  ...
  direction = direction.add(separation.mul(layout.$.fishBehavior.separationStr));
  direction = direction.add(alignment.mul(layout.$.fishBehavior.alignmentStr));
  direction = direction.add(cohesion.mul(layout.$.fishBehavior.cohesionStr));
  direction = direction.add(wallRepulsion.mul(p.fishWallRepulsionStrength));
  direction = direction.add(rayRepulsion.mul(p.fishMouseRayRepulsionStrength));
  direction = std.normalize(direction).mul(std.clamp(std.length(fishData.direction), 0, 0.01));

  const translation = direction.mul(d.f32(std.min(999, layout.$.timePassed)) / 8);
