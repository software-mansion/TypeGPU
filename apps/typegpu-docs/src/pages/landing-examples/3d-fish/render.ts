...
export const vertexShader = tgpu['~unstable'].vertexFn({
  ...
  const worldPosition = std.mul(
    ...
    std.mul(
      yawMatrix,
      std.mul(pitchMatrix, std.mul(scaleMatrix, d.vec4f(wavedVertex.position, 1))),
    ),
  );

  // calculate where the normal vector points to
  const worldNormal = std.normalize(
    std.mul(yawMatrix, std.mul(pitchMatrix, d.vec4f(wavedVertex.normal, 1))).xyz,
  );
...
export const fragmentShader = tgpu['~unstable'].fragmentFn({
  ...
  const diffuse = std.mul(std.max(0, cosTheta), std.mul(textureColor, p.lightColor));

  const viewSource = std.normalize(std.sub(layout.$.camera.position.xyz, input.worldPosition));
  ...
  // apply desaturation
  const distanceFromCamera = std.length(std.sub(layout.$.camera.position.xyz, input.worldPosition));
