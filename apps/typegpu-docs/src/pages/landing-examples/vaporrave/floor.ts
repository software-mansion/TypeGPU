...
export const grid = tgpu.fn(
  ...
  const uvNormalized = std.fract(d.vec2f(uv.x, uv.y + time).div(c.GRID_SEP));

  // x^4 + y^4 = 0.5^4
  const diff4 = std.pow(d.vec2f(0.5, 0.5).sub(uvNormalized), d.vec2f(4, 4));
...
export const circles = tgpu.fn(
  ...
  const uvNormalized = std.fract(d.vec2f(uvRotated.x, uvRotated.y).div(c.GRID_SEP));
