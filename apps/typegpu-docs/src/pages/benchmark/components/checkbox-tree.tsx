...
import { identifierOf, selectedTestsAtom, type Suite, type TestIdentifier } from '../suites.ts';
...
export function SuiteCheckbox(props: { suiteName: string; suite: Suite }) {
  ...
  const childrenIdentifiers: TestIdentifier[] = useMemo(
    () => Object.keys(suite.tests).map((testName) => identifierOf(suiteName, testName)),
    [suite, suiteName],
  );
  ...
  return (
    <div>
      <div className="flex items-center">
        ...
        >
          <span className={cs(opened ? 'scale-y-[-1]' : '', 'inline-block')}>▽</span>
        </button>
        <StylizedCheckbox
          state={state}
          onChange={() => {
            const newSelected = selected.filter((item) => !childrenIdentifiers.includes(item));
            if (state !== 'checked') {
...
function StylizedCheckbox(props: { state: CheckboxState; onChange: () => void }) {
      ...
      <label className="relative flex cursor-pointer items-center">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-white opacity-0 peer-checked:opacity-100">
          <svg
            ...
            viewBox="0 2 100 102"
          >
            <path d="M80 20 L40 78 L20 60" stroke="white" strokeWidth="15" fill="none" />
          </svg>
        </span>
        <span className="absolute inset-0 flex items-center justify-center text-white opacity-0 peer-indeterminate:opacity-100">
          <svg
            ...
            viewBox="0 2 100 102"
          >
            <line x1="20" y1="50" x2="80" y2="50" stroke="white" strokeWidth="15" fill="none" />
          </svg>
        </span>
      </label>
    </div>
