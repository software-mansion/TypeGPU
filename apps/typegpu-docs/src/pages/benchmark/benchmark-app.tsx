...
import { BenchmarkFallback, BenchmarkResults } from './components/benchmark-results.tsx';
...
import { getFilteredSuites, selectedTestsAtom, type Suite, unfilteredSuites } from './suites.ts';
        ...
        <div className="w-full">
          ...
          <div className="w-full">
            {Object.entries(unfilteredSuites).map(([suiteName, suite]) => (
              <SuiteCheckbox suiteName={suiteName} suite={suite} key={suiteName} />
            ))}
          </div>
        </div>
        <button
