...
of the process. We go to extremes to ensure that turning even a single buffer into our typed variant improves the developer experience, and does not require changes
...
and the raw WebGPU resource used underneath.
...
const aBuffer = root.createBuffer(Schema, rawBuffer);

// Updates `rawBuffer` underneath.
aBuffer.write([d.vec3f(1, 2, 3), d.vec3f(4, 5, 6)]);

// Interpreting the raw bytes in `rawBuffer` as easier
// to work with JS values.
const values = await aBuffer.read(); // => d.v3f[]
```
...
Let us deliver on our promise and show you the small code changes necessary to adopt TypeGPU, and the benefits you can gain at each step.
...
##### #1: Define a schema for a buffer's contents
...
- **Automatic sizing** - schemas understand WebGPU memory layout rules, therefore can estimate the required size
  of a buffer.

##### #2: Wrap the buffer in a typed shell
...
##### #3: Let TypeGPU create the buffer
...
##### #1: Replace the WebGPU API call with a typed variant
...
##### #2: Create bind groups from typed layouts
```diff lang="ts"
...
- **Autocomplete** - the IDE can suggest what resources need to be passed in, and what their data type should be.
...
+    },
+    // or extraDependencies: { ...layout.bound },
+  }),
});
```
...
- **Single source of truth** - typed bind group layouts not only describe JS behavior, but WGSL behavior as well. This
