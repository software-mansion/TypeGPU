...
A plain callback throws; use [`disableWorklets`](#opting-out) to run frame callbacks on the RN thread instead.
...
:::note
Definitions created dynamically (inside components or worklets) cannot be captured for use on another runtime.
Keep them at module scope in files covered by `importForwarding`.
