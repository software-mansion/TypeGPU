...
Thanks to the [react-native-webgpu](https://github.com/wcandillon/react-native-webgpu) package,
...
```sh
npm i react-native-webgpu typegpu @typegpu/react
...
export function Triangle() {
  ...
  useFrame(() => {
    ...
    // A react-native-webgpu requirement for flushing the rendered effect
    ctx.present?.();
  });
