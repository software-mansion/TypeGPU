...
In this tutorial, we will explore the capabilities of TypeGPU by transforming a simple program that draws a triangle on the screen into a boids simulation.
While creating the boids simulation, we will learn about the following TypeGPU concepts:

- Creating and using a TypeGPU runtime
- How to write shader code in TypeGPU
...
Along the way there will be some places when you want to follow a different path than the one in the tutorial. Feel free to do so!
There will be some checkpoints where you can compare your code with the tutorial code and see if it works as expected.
The checkpoints will also include comparisons with WebGPU code, so you can see how TypeGPU helps you write less boilerplate code.
...
First, let's adapt our triangle information struct to include velocity, not just position.
...
<Aside>
  ...
  This will also make our struct have 16 byte alignment, which will allow us to use it as a uniform buffer.
  All the calls to `.allowReadonlyStorage()` and `.asReadonlyStorage()` will be replaced with `.allowUniform()` and `.asUniform()`.
  This will make the memory faster but will limit us to 4096 triangles. You don't need to change that as ReadonlyStorage will work just fine.
</Aside>

Let's also create a helper function that will calculate triangle rotation based on its velocity.
...
<Aside type="caution">
  Now that our triangles will move in less predictable ways, we need to add some more clipping logic.
  ```diff lang=ts
  +  if (instanceInfo.position[0] < -1.0 - triangleSize) {
  +    instanceInfo.position[0] = 1.0 + triangleSize;
  +  }
...
The last step of our mini-rework is to update the initialization of the triangles to include velocity.
...
Well, that was a lot of changes! But now all that's left to do is to manipulate the velocity of our triangles in a way that they move in a flocking-like behavior.
Boids work by following three simple rules:
...
<Aside type="caution">
  The following snippets are just the logic for the rules. They are separated for better readability. Later we will combine them into a single code block and add them to the shader.
</Aside>
...
2. Add the logic for alignment:
   ...
   `;
   ```
    Alignment is implemented in a similar way to separation. We create a vector to store the combined alignment forces, which are just the velocities of the other triangles.
    This time, we also keep track of the number of triangles that are within the alignment distance. This is used to normalize the alignment vector.
...
3. Add the logic for cohesion:
   ...
   `;
   ```
    Cohesion basically reduces to calculating the center of mass of the other triangles and moving towards it. After calculating the sum of the positions of the other triangles within the cohesion distance, we normalize the vector and subtract the current position to get the direction to move towards.
...
<Aside type="tip">
  The boids logic is the most subjective part of the implementation. Feel free to experiment with the parameters and the rules to get different flocking behaviors.
</Aside>

To properly see the flocking behavior, we will increase the number of triangles to 500 and reduce their size:
...
Thats it! You have implemented the boids flocking algorithm in WebGPU. Let's see how our code looks and compare it to WebGPU:
