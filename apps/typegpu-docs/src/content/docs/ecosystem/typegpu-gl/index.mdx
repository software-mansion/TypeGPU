...
The WebGL 2 backend does not implement the full TypeGPU API. It currently supports vertex and fragment render pipelines, constants, helper functions, uniforms, vertex and index buffers, depth attachments, and a focused subset of 2D textures. Compute shaders, storage buffers, bind groups, and other WebGPU-only features are not supported.
:::
...
This example keeps its small geometry in TypeGPU constants. For larger meshes, the fallback also supports floating-point vertex buffers and `u16`/`u32` index buffers.
...
The fallback supports single-layer, single-sample 2D textures. A texture can be filled from the CPU, used as a render attachment, and sampled by another render pipeline:
...
const surfaceTarget = surface.createView('render');
const surfaceTexture = surface.createView();
const sampler = root.createSampler({
  magFilter: 'nearest',
  minFilter: 'nearest',
});

// The pipeline target format must match the attached surface texture.
const firstPipeline = root.createRenderPipeline({
  vertex: fullscreenVertex,
  fragment: firstFragment,
  targets: { format: 'rgba8unorm' },
});

firstPipeline.withColorAttachment({ view: surfaceTarget }).draw(3);

const secondPipeline = root.createRenderPipeline({
  vertex: fullscreenVertex,
  fragment: ({ uv }) => {
    'use gpu';
    return std.textureSample(surfaceTexture.$, sampler.$, uv);
  },
});

secondPipeline.withColorAttachment({ view: context }).draw(3);
```

See the complete [Textures (WebGL fallback) example](/TypeGPU/examples/#example=simple--texture-gl), which uploads a small CPU-side palette, renders it into a surface texture, and samples that surface in a second pass.

The fallback normalizes WebGL's framebuffer texture orientation when a rendered texture is sampled, so the same UV coordinates produce the same orientation on the WebGL and WebGPU backends. CPU texture uploads keep their usual TypeGPU orientation.

Supported formats are `r8unorm`, `rg8unorm`, `rgba8unorm`, `rgba8unorm-srgb`, `rgba16float`, and `rgba32float`. Sampled views support `std.textureSample`, `std.textureSampleBias`, `std.textureSampleLevel`, `std.textureLoad`, and `std.textureDimensions`. `depth24plus` is also supported for depth attachments. Multisampled, layered, 1D/3D, storage, and comparison textures remain unsupported.
...
The fallback root supports shader-driven effects and triangle meshes with changing state in uniforms. Author portable shaders in TypeScript; raw WGSL bodies are not translated into GLSL.

| Feature                                            | WebGL 2 fallback                                                               |
| -------------------------------------------------- | ------------------------------------------------------------------------------ |
| Vertex and fragment functions                      | Supported                                                                      |
| TypeGPU constants and helper functions             | Supported                                                                      |
| Uniforms                                           | Scalars, floating-point vectors/matrices, and structs/arrays of these schemas |
| Non-indexed triangle draws                         | Supported                                                                      |
| Vertex and index buffers                           | `float32`/`float32x2`/`float32x3`/`float32x4` attributes; `u16`/`u32` indices     |
| Indexed and instanced triangle draws               | Supported; nonzero `baseVertex` and `firstInstance` are unsupported             |
| Depth attachments                                  | `depth24plus`, depth comparison, writes, and clear/load operations               |
| 2D textures and samplers                           | CPU uploads, color attachments, sampling, and mipmap generation                |
