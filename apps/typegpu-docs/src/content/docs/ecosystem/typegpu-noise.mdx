...
- `normal` - returns a number based on the normal (Gaussian) distribution with a given mean (`mu`) and standard deviation (`sigma`). `sigma` must be `>= 0`
- `cauchy` - returns a number based on the Cauchy distribution with a given origin point (`x0`) and scale parameter (`gamma`). `gamma` must be `>= 0`
