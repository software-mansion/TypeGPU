...
using a set of `randf.seedN`, `randf.iSeedN` functions, where `N` is the number of components our seed has. `randf.seedN` functions are designed to work with floating point numbers, while `randf.iSeedN` functions are designed to work with integer numbers. For details, how to get the best results, check the [generators section](/TypeGPU/ecosystem/typegpu-noise/#generators)
...
## Generators
Seeding generators is hard. The most common method is to generate a seed on the CPU and pass it to the GPU. But we can also make use of thread's `id` to seed the generator. Each generator operates best on different seed domain.
- 1 bpeter - most robust, foolproof, seed must be in range [0, 100000] and we will be fine
- 2 hybridtaus -
- 3
...
The default PRNG is `HybridTaus`, from [NVIDIA's developer guide](https://developer.nvidia.com/gpugems/gpugems3/part-vi-gpu-computing/chapter-37-efficient-random-number-generation-and-application).
