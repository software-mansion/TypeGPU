...
:::tip
Due to limited float precision, for large seeds the samples tend to repeat quickly. To avoid this, keep the seed in the range `[0, 1]`. You can achieve this by either dividing thread's `id` by the total number of threads or by using a seed generated on the CPU, which is the most common method for dealing with it.
:::
...
const b = root.createMutable(d.arrayOf(d.f32, 10000000));
...
// ---cut---
const BPETER: StatefulGenerator = (() => {
...
const p = root['~unstable']
  .with(randomGeneratorSlot, BPETER)
  .withCompute(f)
  .createPipeline();
```
