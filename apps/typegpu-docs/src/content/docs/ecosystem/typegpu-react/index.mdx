...
The `useFrame` hook schedules the passed-in callback to be run every frame using `requestAnimationFrame`. The callback receives an object of type `FrameCtx` that contains
the following:
