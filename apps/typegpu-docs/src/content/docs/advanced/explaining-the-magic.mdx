...
Understanding this is not at all necessary for using TypeGPU, but it may help new contributors understand why some things work like they do.
...
async () => {
    ...
    v: 2,
    name: undefined,
    ast: {"params":[],"body":[0,[[2,[7,"counter","$"],"+=",[7,"incrementBy.allModifier","$"]],[2,[7,[7,"counter","$"],"y"],"+=",[7,"incrementBy.yModifier","$"]]]],"externalNames":["counter","incrementBy"]},
...
Most TypeGPU resources can be named with `$name` method.
While it is completely optional, a name can make debugging easier, and it also serves as a primer for identifiers in resulting WGSL.
...
Along with AST, [tinyest-for-wgsl](https://github.com/software-mansion/TypeGPU/blob/main/packages/tinyest-for-wgsl/README.md) also returns a set of externals.
...
```
The same scenario may occur when a function reference resources that are recreated (e.g. a resized buffer).
...
:::caution
Unplugin only transforms operators inside of 'use gpu' functions.
:::
