...
* **With a command encoder**
  Performance callbacks also work when the pipeline is bound to a [command encoder](/TypeGPU/apis/pipelines/#command-encoders-and-passes) with `pipeline.with(encoder)`. The timestamps are resolved as part of that encoder's submission, and the callback fires from `encoder.submit()`. They cannot be used with a pass begun by someone else — a pass writes its timestamps as part of its descriptor, so pass those to `encoder.beginRenderPass` / `encoder.beginComputePass` instead.
