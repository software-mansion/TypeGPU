...
[unplugin-typegpu](https://www.npmjs.com/package/unplugin-typegpu) is a tool for TypeGPU applications,
which automatically transforms the code during build time,
unlocking some new features, optimizations and quality-of-life perks for the developers.
While using the plugin is not required in majority of scenarios, it is highly recommended when writing GPU programs using TypeGPU.

Currently the package includes the following functionalities, with potentially more coming in the future:

- **TGSL functions**

  TypeGPU allows running a subset of JavaScript functions on the GPU. It is achieved by transpiling the JS code into WGSL.
