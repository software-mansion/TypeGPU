...
External textures provide efficient shader access to sources such as video frames.
...
:::caution
A `GPUExternalTexture` represents a current external-image snapshot and may expire.
