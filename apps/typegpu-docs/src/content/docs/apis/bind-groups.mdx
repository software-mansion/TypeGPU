...
External textures provide efficient, zero-copy shader access to sources such as video frames.
...
const externalFrame = root.device.importExternalTexture({
  source: videoElement,
});

const videoGroup = root.createBindGroup(videoLayout, {
  frame: externalFrame,
...
:::caution
A `GPUExternalTexture` represents a current external-image snapshot and [will expire](https://www.w3.org/TR/webgpu/#external-texture-creation).
For animated sources such as video, import and bind the current frame as part of each frame's rendering work instead of retaining one indefinitely.
:::
