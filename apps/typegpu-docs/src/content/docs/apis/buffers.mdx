...
Combined with `startOffset` and `d.memoryLayoutOf`, this lets you write to a precise region of the buffer. If omitted, writing will continue until the end of the provided data or the end of the buffer, whichever comes first.
...
Call `.clear()` to zero the entire buffer:
