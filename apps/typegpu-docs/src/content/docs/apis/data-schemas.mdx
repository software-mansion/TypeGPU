...
In both cases, modifying `s2` results in a change to `s1`.
This is intuitive to JS developers, but it may be also problematic in some cases:
...
function myFn(arg: d.v2u | d.v3u | d.v4u) {
  'use gpu';
  const vec = arg;
  vec.x++; // ❌ cannot modify function's parameter
}
```

In such cases use, `std.copy`.
...
:::note
If the schema of the copied variable is known, it may be used instead.
```ts twoslash
import tgpu, { d, std } from 'typegpu';
// ---cut---
const MyStruct = d.struct({ n: d.u32 });
