...
The default naming scheme is `"strict"`, which gives resolved objects in the shader code names that closely match what they appear as in JS. Automatically given names can be overriden by explicitly calling `.$name`. If the name isn't a valid WGSL identifier, or there is a name conflict, the name is sanitized and appended with a unique suffix. 

Set `names: "random"` to opt into always applying unique suffixes to global declarations, even when the originally given labels are valid WGSL identifiers and don't conflict with each other.

For advanced cases that generate several related modules, an [experimental shared namespace](/TypeGPU/apis/utils/#shared-resolution-namespaces) can preserve naming and declaration state between resolve calls.
