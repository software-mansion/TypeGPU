...
## Clearing and destroying textures

Call `.clear()` to fill every mip level with zero-valued texels, or pass a mip level to clear only that level:

```ts
texture.clear();    // all mip levels
texture.clear(2);   // only mip level 2
texture.clear('all');
```

Clearing uses texture copy operations, so it is unavailable for formats that WebGPU marks as non-copyable.

Call `.destroy()` when the texture is no longer needed.
This releases the underlying `GPUTexture`; subsequent operations on the TypeGPU texture are invalid.
...
Depth comparisons use a distinct sampler type.
...
const shadowView = shadowMap.createView(d.textureDepth2d());
...
const sampleShadow = tgpu.fn([d.vec2f, d.f32], d.f32)((uv, depth) => {
  'use gpu';
  return std.textureSampleCompare(shadowView.$, shadowSampler.$, uv, depth);
});
```
