...
Channel image writes require `.$usage('render')`. The target texture must be 2D and use a format supported by TypeGPU's render-pass write path. Region options apply to the grouped write:
