...
You can utilize [`root.unwrap`](/TypeGPU/apis/roots) to get the raw `GPUVertexBufferLayout` from a typed vertex layout. It will automatically calculate the stride and attributes for you, according to the vertex layout you provided.
