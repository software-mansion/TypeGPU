...
However, if both branches are known to have no side-effects, then condition can be a runtime value.
:::
...
### `possibleSideEffects`
The fourth optional parameter `possibleSideEffects` is an indicator, whether generating this snippet may produce a WGSL expression with
