...
```ts twoslash
import { tgpu, d, isImmediateVar } from 'typegpu';
const root = await tgpu.init();
const mainVertex = tgpu.vertexFn({ out: { pos: d.builtin.position } })(() => ({ pos: d.vec4f() }));
const mainFragment = tgpu.fragmentFn({ out: d.vec4f })(() => d.vec4f());
const viewProjMatrix = d.mat4x4f();
const encoder = root['~unstable'].createCommandEncoder();
const pass = encoder.beginRenderPass({ colorAttachments: [] });
// ---cut---
const viewProj = tgpu.accessor(d.mat4x4f);

const viewProjSource = root.enabledWgslLanguageFeatures.has('immediate_address_space')
  ? tgpu['~unstable'].immediateVar(d.mat4x4f)
  : root.createUniform(d.mat4x4f);

const pipeline = root
  .with(viewProj, viewProjSource)
  .createRenderPipeline({ vertex: mainVertex, fragment: mainFragment });

// per frame:
if (isImmediateVar(viewProjSource)) {
  pass.setImmediates(viewProjSource, viewProjMatrix);
} else {
  viewProjSource.write(viewProjMatrix);
}
```
