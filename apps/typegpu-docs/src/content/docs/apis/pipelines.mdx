...
In both cases, the pipeline, bind groups and vertex buffers are applied lazily when a draw call is recorded, and only if they changed since the previous one.
...
- `root.unwrap(encoder)` and `root.unwrap(pass)` return the raw `GPUCommandEncoder`, `GPURenderPassEncoder` or `GPUComputePassEncoder`, which can be used e.g. for buffer copies. Commands recorded this way are invisible to TypeGPU, so after unwrapping a pass, every draw applies its full state again.
