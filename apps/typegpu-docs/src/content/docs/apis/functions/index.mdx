...
The available builtins depend on the shader stage and whether the value is an input or output:

| Stage | Direction | Builtins |
| --- | --- | --- |
| Compute | Input | `localInvocationId: vec3u`, `localInvocationIndex: u32`, `globalInvocationId: vec3u`, `globalInvocationIndex: u32`, `workgroupId: vec3u`, `workgroupIndex: u32`, `numWorkgroups: vec3u`, `subgroupInvocationId: u32`, `subgroupSize: u32`, `subgroupId: u32`, `numSubgroups: u32` |
| Vertex | Input | `vertexIndex: u32`, `instanceIndex: u32` |
| Vertex | Output | `clipDistances: array<u32, 8>`, `position: vec4f` |
| Fragment | Input | `position: vec4f`, `frontFacing: bool`, `primitiveIndex: u32`, `sampleIndex: u32`, `sampleMask: u32`, `subgroupInvocationId: u32`, `subgroupSize: u32` |
| Fragment | Output | `fragDepth: f32`, `sampleMask: u32` |

:::note
Subgroup builtins require the [`subgroups` feature](/TypeGPU/advanced/enabling-features), and `clipDistances` requires the [`clip-distances` feature](/TypeGPU/advanced/enabling-features).
:::

`globalInvocationIndex` and `workgroupIndex` are scalar, linear counterparts to the three-dimensional ID builtins.
They are convenient when indexing flat buffers:

```ts
const mainCompute = tgpu.computeFn({
  in: { globalIndex: d.builtin.globalInvocationIndex },
  workgroupSize: [64],
})(({ globalIndex }) => {
  values.$[globalIndex] *= 2;
});
```
...
In WGSL-implemented functions, input and output structs of the given function can be referenced as `In` and `Out` respectively.
