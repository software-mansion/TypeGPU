...
Scalar, vector, matrix, struct and fixed-size array schemas can be called with no arguments to get a default value of the type represented by the schema. Arrays are a WIP.
...
Scalar, vector, matrix, struct and fixed-size array schemas can be called with an instance of the type represented by the schema to create a deep copy of the argument.
