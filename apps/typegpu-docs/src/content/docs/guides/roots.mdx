...
| --- | --- |
| <div className="w-max">`root.createBuffer`</div> | Creates a typed buffer with a given data-type and, optionally, an initial value. More information in [the next guide](/TypeGPU/guides/tgpu-buffer-api). |

## Unwrapping resources

There are times where a typed resources needs to be used by a vanilla WebGPU API. To retrieve the raw
untyped value of a typed resource, use the `root.unwrap` function.

| Function | Description |
| --- | --- |
| `root.unwrap(aTypedBuffer)` | Returns a `GPUBuffer`. |
| `root.unwrap(aTypedBindGroupLayout)` | Returns a `GPUBindGroupLayout`. |
| `root.unwrap(aTypedBindGroup)` | Returns a `GPUBindGroup`. |
...
:::note
When designing an architecture from stratch with TypeGPU in mind, passing a `root` around instead of a `device` is the prefered pattern, since
`root.device` can act as an escape-hatch for vanilla WebGPU functionality.
:::

You can copy and paste the utility below that implement a basic global cache for roots.
