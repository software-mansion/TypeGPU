...
If we wanted to make use of TypeGPU's type-safe features with these shaders, we could create the appropriate definitions by hand.
This, however, can be a tedious and difficult to maintain process.
Instead, we can use TypeGPU CLI to generate the necessary TypeScript definitions for us.
...
<FileTree>
  - computeBoids
    - shaders
      - updateSprites.wgsl
      - sprite.wgsl
    - <FileDiff> definitions </FileDiff>
      - <FileDiff> updateSprites.ts </FileDiff>
      - <FileDiff> sprite.ts </FileDiff>
</FileTree>
</div>
...
For specific usage details, read the Usage .
...
<FileTree>
  - path/to
      - shader1.wgsl
      - <FileDiff> shader1.ts </FileDiff>
      - shader2.wgsl
      - <FileDiff> shader2.ts </FileDiff>
      - example
          - shader3.wgsl
</FileTree>
</div>
...
<FileTree>
  - path/to
      - shader.wgsl
  - <FileDiff> different/path </FileDiff>
      - <FileDiff> output.ts </FileDiff>
</FileTree>
</div>
...
<FileTree>
  - src
      ...
      - shader2.wgsl
  - <FileDiff> output </FileDiff>
      - <FileDiff> shader1.ts </FileDiff> \<\-\- name conflict
      - <FileDiff> shader1.ts </FileDiff> \<\-\- name conflict
      - <FileDiff> shader2.ts </FileDiff>
</FileTree>
</div>
