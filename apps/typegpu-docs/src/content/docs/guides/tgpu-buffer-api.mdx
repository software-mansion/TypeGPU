---
title: TypeGPU buffer API
description: A guide on how to use the TypeGPU buffer API.
---

This guide will show you how to use the TypeGPU buffer API to allow for type-safe and convenient read and write operations on WebGPU resources.
We will explore how to adapt existing WebGPU code to use the buffer API, and how to use the it to create new WebGPU resources.

## What is the TypeGPU buffer API?

TypeGPU buffer API is a set of tools for WebGPU that allow you to create and manage typed buffers.
...
## Using the TypeGPU buffer API

### Creating a buffer

import { Aside } from '@astrojs/starlight/components';

To create a buffer using the TypeGPU buffer API, you will need to define the buffer type using one of the data types provided by the `typegpu/data` module.
...
<Aside>
    ...
    Remember that this also means you are responsible for the buffer's lifecycle.
    ```ts
    // WgslBuffer<U32>
...
When creating a buffer using the TypeGPU buffer API, you can use one of the available builder methods to add usage flags to the buffer.
...
Let's take a look at how we can supercharge our existing WebGPU code with the TypeGPU buffer API.
...
Let's see how we can improve this code using the buffer API.
...
In this approach we will remove the existing buffer and replace it with a buffer created using the TypeGPU buffer API.
