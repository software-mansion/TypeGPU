...
<Aside>
    ...
    This marks the buffer as unmanaged and allows you to use it in the unmanaged API.
    ```ts
    // WgslBuffer<U32>
...
Let's take a look at how we can supercharge our existing WebGPU code with the unmanaged API.
...
Let's see how we can improve this code using the unmanaged API.
...
const colorBuffer = tgpu.createBuffer(ColorPair).$device(device);

const updateColors = (
    colorFrom: [number, number, number],
    colorTo: [number, number, number]
) => {
    // The write function will not allow us to write data that does not match the buffer type
...
As we can see in the example above, the `updateColors` function is now much simpler to the point of becoming redundant, so let's remove it. 
The write function will not allow us to write data that does not match the buffer type, so we can be sure that the data is written correctly.
