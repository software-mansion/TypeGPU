...
<Tabs>
    ...
    new Float32Array(data).set([0.0, 0.0, 0.0]);
    new Uint32Array(data).set([100], 3);
    buffer.unmap();
    ...
    device.queue.copyBufferToBuffer(buffer, 0, stagingBuffer, 0, 16);

    // Reading the data
    await stagingBuffer.mapAsync(GPUMapMode.READ);
    const data = new Float32Array(stagingBuffer.getMappedRange(), 0, 3);
    const health = new Uint32Array(stagingBuffer.getMappedRange(), 12, 1)[0];
    const value = { position: data, health };
    ...
    // Creating and initializing a buffer. The type hint is WgslBuffer<U32>
