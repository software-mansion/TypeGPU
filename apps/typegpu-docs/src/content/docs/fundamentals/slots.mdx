...
Similarly to bind group layouts, you can think of slots as of typed "holes" in tgpu shaders, that can be filled in later on from the outside.
The main differences between slots and bound resources include the following:

- Slots are filled in during or before the shader module compile time, instead of just before the shader execution starts. 
- Slots can contain not only buffers, but also basically anything that is allowed in TGSL, even `TgpuFn`s. 
...
A slot is created using the `slot()` function and can optionally take a default value:
...
:::caution
Slot values can only be accessed inside of tgpu functions.
...
The first way is to call the `with` method on the `TgpuFn` instance:
...
:::note
Static code analysis does not incorporate whether all slots were filled or not.
If you forget about filling one of the slots, you will instead get a descriptive runtime error.
:::


## Inversion of control

With slots, libraries can expose customization points for their users. 
They enable behavioral changes to internal library functionality while maintaining type safety and performance.
...
{/*
TODO: readd and rewrite this section back when we actually prune branches.
...
It is possible to use slots even in tgpu functions that are implemented in wgsl.
In this case, the most common way to do so is with `string` slots:
