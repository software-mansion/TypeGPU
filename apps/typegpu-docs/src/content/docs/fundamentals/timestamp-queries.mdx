...
:::tip
To use performance listeners and timestamp queries, you must enable the `timestamp-query` feature on your GPU device. See [Enabling Features](/TypeGPU/fundamentals/enabling-features) for details.
:::

TypeGPU offers two ways to employ timestamp queries:

1. **High-level API** via the pipeline’s `.withPerformanceListener()` method  
...
Rather than managing query sets yourself, you can measure shader dispatch times easily by chaining `.withPerformanceListener()` onto a compute or render pipeline:

```ts
const pipeline = root['~unstable']
  .withCompute(computeShader)
  .createPipeline()
  .withPerformanceListener((start, end) => {
...
* **Multiple listeners**
  If you call `.withPerformanceListener()` more than once, only the *last* callback is used. If your timing logic doesn’t change, attach it a single time *after* creating the pipeline rather than on every dispatch.

* **Automatic query set**
  If you haven’t provided a `TgpuQuerySet` before calling `.withPerformanceListener()`, TypeGPU will allocate one for you along with the necessary resolve buffers.
...
## Combining `TgpuQuerySet` with Performance Listeners

You can use a custom query set and still attach a performance listener. TypeGPU will write to the indices you specify, then resolve and read the full set for the listener:
...
The listener respects your chosen indices but still resolves and reads the entire query set under the hood.
