...
Some advanced GPU capabilities—such as timestamp queries or subgroup operations—must be explicitly requested before use. TypeGPU passes your configuration directly to the WebGPU API, with a convenient extension for marking features as optional.
...
After `tgpu.init()` completes, inspect the `root.enabledFeatures` set to see which features were actually activated:
