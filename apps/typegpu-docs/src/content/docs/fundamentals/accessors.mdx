...
When writing reusable GPU functions in TypeGPU, you often need to reference "some resource" — a buffer, a variable, a texture — without knowing in advance which specific resource will be used.
[Slots](/fundamentals/slots) solve this for arbitrary JavaScript values, but GPU resources require more: they must be declared with a data schema, produce correct WGSL bindings, and support field access through struct types.

**Accessors** fill this role. They are schema-aware, typed placeholders specifically designed for GPU resources.
An accessor knows the data type of the resource it represents (`d.vec3f`, `d.struct(...)`, etc.), and TypeGPU validates that whatever is bound to it matches that schema.
...
const getGreenColor = getColor.with(colorAccess, d.vec3f(0, 1, 0));
```
...
A read-only `tgpu.accessor` does not support assignment — use `tgpu.mutableAccessor` when mutation is needed.
...
| **Field access through `.$`** | Yes — `accessor.$.field` works for struct schemas | No (a slot returning a struct doesn't support field access) | N/A |
| **Mutation support** | `tgpu.mutableAccessor` | No | Yes (storage buffers) |
...
**Choose slots** when you want to inject arbitrary JavaScript values or TypeGPU functions that affect shader logic — for example, a gravity function or a boolean flag.

**Choose accessors** when the placeholder must be a GPU resource with a data schema, especially when you need deep field access or mutation, or when you want a library to expose a resource customization point.
...
For more on slots, see the [Slots guide](/fundamentals/slots).
For more on bind groups, see the [Bind Groups guide](/fundamentals/bind-groups).
