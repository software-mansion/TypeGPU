...
:::note
If you already are using TypeGPU pipelines, the `.draw()` and `.dispatchWorkgroups()` methods perform the resolution and shader compilation under the hood, eliminating the need for manual resolves.
:::
...
:::tip
This variant of `tgpu.resolve` can often be omitted with [WGSL-implemented functions](/TypeGPU/fundamentals/functions/#implementing-functions-in-wgsl).
...
:::note
You can access [bindGroupLayout](/TypeGPU/fundamentals/bind-groups) entries via the `layout.$` property.

```ts
// Requires using `layout.$.resource` in the WGSL code
externals: {
  layout,
}

// Allows using resource names in the WGSL code
externals: {
...
// As long as the names in the WGSL code match the
