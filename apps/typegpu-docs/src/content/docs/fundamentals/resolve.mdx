...
:::note
If you already are using TypeGPU pipelines, omitting the manual resolve by using the `.draw()` and `.dispatchWorkgroups()` may be the simplest approach.
:::

:::tip
To resolve all items from a layout, you can use `tgpu.resolve([...Object.values(layout.$)])`.
:::
...
:::tip
This variant of `tgpu.resolve` can often be omitted using [WGSL-implemented functions](/TypeGPU/fundamentals/functions/#implementing-functions-in-wgsl).
...
As you may note, in this case `tgpu.resolve` uses the structure of the `external` property to detect that `buffers.time`, `buffers.range` and `offset` need to be replaced, and that their definitions should be included.
...
:::note
To resolve bindings you can access each entry of a [bindGroupLayout](/TypeGPU/fundamentals/bind-groups) via the `layout.$` property.

```ts
  externals: {
    lightSource: layout.$.lightSource,
    sampling: layout.$.sampling,
    bgTexture: layout.$.bgTexture,
  }
...
When items are being resolved, they are given new names based on their name and the specified naming scheme (`names` parameter in options).
