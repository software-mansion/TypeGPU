...
For these cases, you can use `tgpu.resolveWithContext`, which has the same input API as `tgpu.resolve`, but in addition to code, it also returns information about the layouts used. `tgpu.resolveWithContext` returns an object with 3 props:

- `code` - the resolved code,
- `usedBindGroupLayouts` - a list of used `tgpu.bindGroupLayout`s,
...
```ts
const myFixedBuffer = root.createMutable(d.u32);

let { code, usedBindGroupLayouts, catchall } = tgpu.resolveWithContext({
  template: `
    fn exampleFn() {
      myFixedBuffer += 1;
    }
  `,
  externals: { myFixedBuffer },
});
