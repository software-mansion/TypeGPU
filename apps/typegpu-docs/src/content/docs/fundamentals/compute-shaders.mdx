...
<Tabs>
  <TabItem label="Laid Out Resources">
    ...
    const simulate = root.createGuardedComputePipeline((x) => {
      'use gpu';
      const { position, velocity } = simulationLayout.$.particles[x];
      simulationLayout.$.particles[x].position =
        position + velocity * simulationLayout.$.deltaTime;
    });
  ...
  <TabItem label="Fixed Resources">
    ...
    const simulate = root.createGuardedComputePipeline((x) => {
      'use gpu';
      const { position, velocity } = particlesMutable.$[x];
      particlesMutable.$[x].position =
        position + velocity * deltaTimeUniform.$;
    });
