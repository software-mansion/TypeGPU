...
Now the same simulation step can be written in either style. The Fixed Resources version closes over one particular set of resources, while the Laid Out Resources version keeps the pipeline reusable and supplies the resources when dispatching.
...
The tradeoff is a little more setup, but the shader no longer closes over one concrete resource. It can be separated cleanly from runtime resource setup and reused with different bind groups.

:::tip
Layouts are the only way to swap resources for a shader after pipeline creation, but they are not the only way to create modular shaders. We will cover [slots](/TypeGPU/apis/slots/) and [accessors](/TypeGPU/apis/accessors/) in later sections. They provide a similar way of defining resource-independent shaders, but each change requires creating a new shader module.
:::
