...
#### Resources that fit this layout entry
...
To be able to sample a texture in a shader, create a property of with the value matching:
```ts
{
...
To be able to operate on textures more directly in a shader, create a property of with the value matching:
```ts
{
