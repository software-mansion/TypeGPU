...
## Index Buffers
TypeGPU also allows for the use of index buffers. An index buffer is a buffer containing a sequence of `u32` indices, which indicate the order in which vertices from a vertex buffer ought to be processed. This might be particulary useful for rendering geometry, where vertices are often shared between multiple primitives (e.g., triangles), as it avoids duplicating vertex data.
When using an index buffer, instead of explicitly listing every vertex for every triangle, you provide a list of unique vertices in a vertex buffer and then use the index buffer to define how these vertices form triangles (or other primitives).

To leverage indexed rendering with TypeGPU, you'll create a buffer with a schema for integer types (like d.u32 for unsigned 32-bit integers, which are commonly used for indices) and declare its usage as an index buffer.
```ts twoslash
import tgpu from 'typegpu';
import * as d from 'typegpu/data';
