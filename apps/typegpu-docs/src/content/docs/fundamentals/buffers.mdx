...
You can explicitly create bind groups and bind your buffers to them.
...
const bindGroupLayout = tgpu.bindGroupLayout({
  points: { storage: d.arrayOf(d.vec2i, 100) },
});
...
const mainCompute = tgpu['~unstable'].computeFn({
  ...
  bindGroupLayout.$.points[0] = d.vec2i();
});
...
In many cases, the buffers you attach to shaders remain the same across multiple compute or render operations.
TypeGPU simplifies this process through fixed resources, that from the code perspective appear to be bindless.
Fixed buffers are created using specific root methods.
...
const mainCompute = tgpu['~unstable'].computeFn({
  ...
  // access and modify the bound buffer via the layout
  pointsBuffer.$[0] = d.vec2i();
});

const pipeline = root['~unstable'].withCompute(mainCompute).createPipeline();
```

TypeGPU automatically generates a "catchall" bind group, and populates it with the fixed resources.

You can also use [`resolveWithContext`](/TypeGPU/fundamentals/resolve/#resolvewithcontext) to access the automatically generated bind group and layout that contains your fixed resources.
