...
Memory on the GPU can be allocated and managed through buffers. That way, we can provide WGSL shaders with an additional context, or retrieve the results of parallel computation back to JS. When creating a buffer, a schema for the contained values has to be provided, which allows for:
...
As an example, lets create a buffer for storing particles.
...
structs and arrays. For a comprehensive list of all available schemas, see the [Data Schemas](/TypeGPU/reference/data-schemas) reference.
...
:::caution[WebGPU Interoperability]
If you create a buffer from an existing WebGPU buffer than happens to be mapped, the data will be written directly to the buffer (the buffer will not be unmapped).
