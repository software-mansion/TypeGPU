...
### Permissive vector and matrix inputs
...
For matrix fields and buffers, `.write()` accepts typed matrix instances, plain `number[]`, and `Float32Array`.
...
For **arrays**, a `TypedArray` is treated as a **raw byte copy**: TypeGPU copies the bytes directly without any per-element processing. This is useful when your data is already stored in the padded GPU layout (e.g. output from a previous GPU readback, or a manually laid-out simulation buffer), as it avoids any allocation overhead and becomes a single `memcpy`.
...
### Raw ArrayBuffer writes

`.write()` also accepts a plain `ArrayBuffer` as a zero-overhead path for when you already have the exact GPU-layout bytes (e.g. from a GPU readback, shared memory, or another binary source). TypeGPU copies the bytes directly into the GPU buffer with no interpretation.
