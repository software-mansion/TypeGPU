...
|---|---|---|
| Typed vec instance | `d.vec3f(1, 2, 3)` | Allocates a new object |
| Plain JS tuple | `[1, 2, 3]` | No extra allocation |
| TypedArray | `new Float32Array([1, 2, 3])` | Best for bulk data |

Avoiding vec construction can meaningfully reduce garbage-collector pressure in hot paths such as per-frame updates or simulation ticks.
...
// Flat Float32Array - most efficient, no per-element object allocation:
