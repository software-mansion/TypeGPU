...
The `getColor` function, when resolved to WGSL, includes the definitions of all used external resources:

```wgsl
fn getBlue_1() -> f32 { return 0.941 }

fn getColor_0(alpha: f32) -> vec4f {
  let color = vec4f(0.114, 0.447, getBlue_1(), alpha);
  return color;
}
```
