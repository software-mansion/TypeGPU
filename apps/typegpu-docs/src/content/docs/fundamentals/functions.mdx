...
TypeGPU schemas for these structs are created automatically by the library and their definitions are included when resolving the functions. The names referenced in the implementations do not matter. The schemas can be accessed through dedicated properties on created entry functions (`mainVertex.inputType`, `mainVertex.outputType`, `mainFragment.outputType`).
