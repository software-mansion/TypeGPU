...
TypeGPU also requires functions for quickly defining and running render and compute pipelines.
...
Defining entry functions is similar to regular ones, but is done through dedicated constructors: `tgpu['~unstable'].vertexFn`, `tgpu['~unstable'].fragmentFn`, `tgpu['~unstable'].computeFn`. 
...
After creating tgpu entry functions, you can access their input and output data schemas through dedicated properties:

```ts
mainVertex.inputType //  d.WgslStruct<WithLocations<{}>>
mainVertex.outputType //  d.WgslStruct<WithLocations<{}>>
```
