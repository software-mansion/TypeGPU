...
Creating a render pipeline requires calling a `withVertex` method first, which accepts a `TgpuVertexFn` and vertex attributes.
...
The next step is calling the `withFragment` method, which accepts a `TgpuFragmentFn` and a *targets* argument defining the formats and behaviors of the color targets the pipeline writes to.
