...
The `drawIndexed` is analogous to draw, but takes advantage of the [index buffer](/TypeGPU/fundamentals/buffers/#index-buffers-and-drawindexed-api) to determine the vertices to process. It takes the total count of vertices it is meant to draw an argument.
