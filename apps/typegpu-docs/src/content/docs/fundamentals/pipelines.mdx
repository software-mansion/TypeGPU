...
Creating a render pipeline requires first calling a `withVertex` method, which accepts a tgpu vertexFn and a record of vertex attributes.
If the vertex shader does not use vertex attributes, then the latter argument is `{}`.
It is also possible to use some parts of multiple [tgpu.vertexLayout](/TypeGPU/fundamentals/vertex-layouts) by specifying selected attributes in the passed record.
The compatibility between vertex input types and vertex attribute formats is performed at the type level.
...
Before executing pipelines, it is necessary to bind all of the utilized resources, like bind groups, vertex buffers
...
The higher-level API has several limitations, therefore another way of executing pipelines is exposed, for some custom, more demanding scenarios.
