...
For vector WGSL types `vec2f`, `vec2u`, `vec2i`, `vec2f` and `vec2<bool>`, TypeGPU provides `d.vec2f`, `d.vec2u`, `d.vec2i`, `d.vec2f` and `d.vec2b` schemas respectively. Analogously for 3 and 4 components.
...
We define the `Circle` struct, in a similar way to how we would in WGSL, and then we can use it to validate and cast our data values. 
