...
Apart from defining buffers and functions, schemas can also be called to create instances of the types they represent.
...
```
For the WGSL and JS functions to return consistent results, TypeGPU wraps all divisions in `f32` casts.
:::
...
const vec1mat = std.mul(vec2, mat); // d.vec3f(8, 20, 48);
//    ^?

const comp = std.le(d.vec3f(2, 3, 4), d.vec3f(3)); // d.vec3b(true, true, false)
//    ^?
```
...
Remember, that in case of scalars, you can still use `+`, `<=`, `!` etc., 
...
const multiplication = v1.mul(v2).mul(v2); // same as std.mul(std.mul(v1, v2), v2)
```
...
The `Boid` schema also allows for default value creation and code autocompletion.
...
On JS side, s2 is a reference to s1 and therefore s1 is modified when s2 is modified.
On WGSL side, the assignment operator copies the value of s1, and thus s1 is not modified later on.
