...
:::caution
Calling a pipeline with a performance callback always flushes the command encoder.
Similarly, read–write operations on buffers also flush the command encoder.
...
const renderPipelineWithPerformanceCallback = renderPipeline
...
const render = () => {
  ...
  // force flush caused by renderPipelineWithPerformanceCallback
  // new command encoder
};
