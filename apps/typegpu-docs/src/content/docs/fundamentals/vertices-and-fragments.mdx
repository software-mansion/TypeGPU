...
1. The **vertex** function runs once per vertex and outputs a position for it.
...
Each fragment receives values that are **interpolated** across the primitive based on its position.
...
Vertices are grouped in threes, and the fragment runs for every pixel covered by the triangle they define.
...
For larger meshes, store the positions in a buffer instead of inlining them in the shader.
...
That distance check is a **signed distance function** (SDF) — a way to describe shapes using functions instead of geometry.
For more, see [Inigo Quilez's 2D distance functions](https://iquilezles.org/articles/distfunctions2d/).
