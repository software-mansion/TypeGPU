...
:::caution
Calling `Math` functions like (`Math.sin`) inside TypeGPU functions should be avoided.
- not all `Math` functions are supported,
- using `Math` functions may lead to unexpected type conversions.

All supported `Math` functions fallback to their `std` equivalents, and it is advised to use `std` directly.
:::
