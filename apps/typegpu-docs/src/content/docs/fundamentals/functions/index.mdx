...
Then the actual WGSL implementation is passed in as an argument to shell invocation.
...
const mainFragment = tgpu['~unstable']
  .fragmentFn({ in: { uv: d.vec2f }, out: d.vec4f })(/* wgsl */ `(input: FragmentInput) -> @location(0) vec4f {
