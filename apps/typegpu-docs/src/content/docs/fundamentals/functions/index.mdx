...
TypeGPU offers many quality-of-life features that help with building complex WGSL shaders, like the ability to split functionality across
different files, or to import a shader utility function from a library. The approach that TypeGPU takes aims for composability:

- Each definition in WGSL is represented as its own TypeGPU resource -- a typed wrapper representing some kind of WGSL resource (e.g., `TgpuFn`, `TgpuBuffer`, `TgpuVar`).
- Resources can reference outside resources, like buffers, bind group layouts, or user-defined tgpu functions.
...
The shell constructor `tgpu.fn` makes use of [TypeGPU schemas](/TypeGPU/fundamentals/data-schemas) -- objects representing different WGSL data types, that help us with generating WGSL at runtime.
...
TypeGPU has split the wgsl attribute syntax into different places to provide type safety and to simplify the shader creation and dispatching process.
For example, all groups and bindings are assigned automatically.
On the other hand, defining entry functions is not done by annotating `TgpuFn` with attributes, but is achieved through dedicated shell constructors instead:

- `tgpu['~unstable'].vertexFn`,
- `tgpu['~unstable'].fragmentFn`,
- `tgpu['~unstable'].computeFn`.

Shell constructors of entry functions accept an object that describes the IO as well as other relevant parameters, such as workgroup size.

Builtin inter-stage inputs and outputs are available on the `d.builtin` object. 
...
In the case of entry functions, you should not include the WGSL function header in the code string.
Instead, the WGSL implementations can access these arguments as if they were passed in via structs.
...
Input values are accessible through the struct named `in`, while the automatically created structs for input and output shall be referenced in implementation as `In` and `Out` respectively.

If you wish to, you can wrap any schema in an IO struct in a `d.location` decorator, however TypeGPU tries to assign locations automatically.
Other potentially useful decorators include `d.size` and `d.align`, corresponding to `@size` and `@align` wgsl attributes.
