...
- Resources can reference outside resources, like buffers, bind group layouts, or user-defined TypeGPU functions.
- Resources can be imported and exported like any other JavaScript value.

## Defining a function

In order to construct a TypeGPU function, you need to start with defining its shell -- an object holding only the input and output types of a TypeGPU function.
...
For describing the I/O of an entry point function, we use `IORecord`s --
JS objects, where keys represent the names of the arguments,
and values represent the types of the arguments.
...
However, if you wish to, you can still wrap any schema in a `d.location` decorator.
...
In WGSL implementation, input and output structs of the given function shall be referenced as `In` and `Out` respectively.
Headers in WGSL implementations should be omitted, all input values are accessible through the struct named `in`.
