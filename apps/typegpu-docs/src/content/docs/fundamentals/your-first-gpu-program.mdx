...
:::note
The size limits listed here are guaranteed minimums. Most devices support higher limits when explicitly requested.
:::
...
This will work correctly (as long as we never want to update the `incrementBy` field), and sometimes it is the right thing to do, especially when the initial data is computed on demand and highly parallelizable. In our current use case, though, it is overkill.
