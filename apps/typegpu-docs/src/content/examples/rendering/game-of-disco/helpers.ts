...
export const unpackVec2u = tgpu['~unstable']
  .fn([d.vec2u], d.vec4f)
  .does((input) => {
...
export const packVec2u = tgpu['~unstable']
  .fn([d.vec4f], d.vec2u)
  .does((input) => {
...
export const normalizeSafely = tgpu['~unstable']
  .fn([d.vec4f], d.vec4f)
  .does((v) => {
...
export const getNormal = tgpu['~unstable']
  .fn([d.vec4f, d.vec4f, d.vec4f, d.u32, d.vec4f], d.vec4f)
  .does((v1, v2, v3, smoothNormals, vertexPos) => {
    if (smoothNormals === 1) {
      return vertexPos;
    }
...
export const calculateMidpoint = tgpu['~unstable']
