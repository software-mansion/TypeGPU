...
export const sandboxModulesAtom = atom(
  async (): Promise<Record<string, SandboxModuleDefinition>> => {
    const localModules = fromEntries(
      await Promise.all(
        entries(allPackagesSrcFiles).map(async ([key, getContent]) =>
          srcFileToModule([key, await getContent()], '../../../../../packages/'),
        ),
      ),
    );
    const nodeModules = fromEntries(
      await Promise.all(
        [...entries(threeModules), ...entries(mediacaptureModules), ...entries(reactModules)].map(
          async ([key, getContent]) =>
