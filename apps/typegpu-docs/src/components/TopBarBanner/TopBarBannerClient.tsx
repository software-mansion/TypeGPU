...
import { MIN_VISIBLE_HEIGHT, cacheKey, varNames, type BannerZone } from './shared.ts';

export type { BannerZone };
export { topBarBannerReservationScript } from './shared.ts';

const DEFAULT_ROTATE_INTERVAL_MS = 4000;
const SLIDE_MS = 700;

const rotateLeft = <T,>(arr: T[], k: number): T[] => arr.slice(k).concat(arr.slice(0, k));
...
const isTransparent = (color: string) =>
  !color || color === 'transparent' || /^rgba?\([^)]*,\s*0\s*\)$/.test(color.replace(/\s+/g, ' '));
...
const extractBgColor = (root: HTMLElement): string | null => {
  const direct = findOpaqueBg([root, ...Array.from(root.querySelectorAll<HTMLElement>('*'))]);
...
export const TopBarBanner = ({
  ...
  const handleZoneChange = useCallback((i: number, state: ZoneState) => {
    ...
    setZoneStates((prev) => {
      ...
      if (cur && cur.height === state.height && cur.hasBanner === state.hasBanner) {
        return prev;
      }
  ...
  useEffect(() => {
    ...
    document.documentElement.dataset.bannerLoaded = anyBanner ? 'true' : 'false';
