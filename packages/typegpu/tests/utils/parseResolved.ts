...
import type { TypeBit } from '#shaderbit';
...
import type { Snippet } from '../../src/tgsl/snippet.ts';
...
export function expectDataTypeOf(cb: () => unknown): Assertion<TypeBit | UnknownData> {
  return expect<TypeBit | UnknownData>(extractSnippetFromFn(cb).dataType);
}
