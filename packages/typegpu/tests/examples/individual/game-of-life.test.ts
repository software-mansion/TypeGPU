  ...
  it('should produce valid code', async ({ device }) => {
    ...
    expect(shaderCodes).toMatchInlineSnapshot(`
      ...
      @compute @workgroup_size(16, 16) fn tiledCompute(_arg_0: tiledCompute_Input) {
        ...
        var tileOrigin = ((vec2f(_arg_0.wgid.xy) * 16) - 1);
      ...
      @fragment fn displayFragment(_arg_0: displayFragment_Input) -> @location(0) vec4f {
        ...
        if (inMinimap) {
          ...
          let edgeDist = sdRoundedBox2d((localUv - 0.5), vec2f(0.5), 0.02f);
          if ((edgeDist > -0.02f)) {
            let alpha = (1f - smoothstep(0, 0.02, edgeDist));
          ...
          if (((dist > -(borderWidth)) && (dist < borderWidth))) {
            ...
            let a = (1f - smoothstep(0, borderWidth, abs(dist)));
        ...
        if (((*zoom).enabled == 1u)) {
          sampleUv = (((_arg_0.uv - 0.5) / (*zoom).level) + clampedCenter);
        }
