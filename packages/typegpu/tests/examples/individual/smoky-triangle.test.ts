  ...
  it('should produce valid code', async ({ device }) => {
    ...
    expect(shaderCodes).toMatchInlineSnapshot(`
      ...
      fn grain(color: vec3f, uv: vec2f) -> vec3f {
        return (color + (sample(vec3f((uv * 200), grainSeed)) * 0.1f));
      }
      ...
      @fragment fn fragmentFn(_arg_0: FragmentIn) -> @location(0) vec4f {
        let t = (time * 0.1f);
        var ouv = ((_arg_0.uv * 5) + vec2f(0f, -(t)));
        var off = (vec2f(sample(vec3f(ouv, t)), (sample(vec3f((ouv * 2), (t + 10f))) * 0.5f)) + -0.1);
        off = tanhVec((off * sharpness));
        var p = (_arg_0.uv + (off * distortion));
        var factor = 0f;
        if ((polarCoords == 1u)) {
          factor = length(((p - vec2f(0.5, 0.30000001192092896)) * 2));
        }
        else {
          factor = ((p.x + p.y) * 0.7f);
        }
