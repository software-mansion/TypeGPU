  ...
  describe('stringifyNode', () => {
    describe('empty containers', () => {
      ...
      it('handles call with no arguments', () => {
        ...
        expect(stringifyNode(node)).toBe('foo()');
      });
    });

    describe('unary operators', () => {
      it('handles ! (logical NOT)', () => {
        expect(stringifyNode([N.unaryExpr, '!', 'x'] satisfies tinyest.UnaryExpression)).toBe('!x');
      });
      ...
      it('handles void', () => {
        expect(stringifyNode([N.unaryExpr, 'void', 'x'] satisfies tinyest.UnaryExpression)).toBe(
          'void x',
        );
      });

      it('handles delete', () => {
        expect(stringifyNode([N.unaryExpr, 'delete', 'x'] satisfies tinyest.UnaryExpression)).toBe(
          'delete x',
        );
      });
      ...
      it('wraps complex inner for word operator', () => {
        ...
        expect(stringifyNode(node)).toBe('typeof (a + b)');
      });

      it('handles post-update --', () => {
        expect(stringifyNode([N.postUpdate, '--', 'i'] satisfies tinyest.PostUpdate)).toBe('i--');
      });

      it('handles pre-update ++', () => {
        expect(stringifyNode([N.preUpdate, '++', 'i'] satisfies tinyest.PreUpdate)).toBe('++i');
      });
    });
    ...
    describe('logical operators', () => {
      it('handles || (logical OR)', () => {
        expect(
          stringifyNode([N.logicalExpr, 'a', '||', 'b'] satisfies tinyest.LogicalExpression),
        ).toBe('a || b');
      });
      ...
      it('wraps left-side nested logical', () => {
        ...
        expect(stringifyNode(node)).toBe('(a && b) || c');
      });
    ...
    describe('binary operators', () => {
      it('handles ** (exponentiation)', () => {
        expect(
          stringifyNode([N.binaryExpr, 'a', '**', 'b'] satisfies tinyest.BinaryExpression),
        ).toBe('a ** b');
      });

      it('handles in', () => {
        expect(
          stringifyNode([N.binaryExpr, 'key', 'in', 'obj'] satisfies tinyest.BinaryExpression),
        ).toBe('key in obj');
      });
    ...
    describe('compound assignment operators', () => {
      ...
      it('handles -=', () => {
        expect(
          stringifyNode([N.assignmentExpr, 'x', '-=', 'y'] satisfies tinyest.AssignmentExpression),
        ).toBe('x -= y');
      });
    ...
    describe('wrapIfComplex behavior', () => {
      it('wraps conditional as binary operand', () => {
        ...
        expect(stringifyNode(node)).toBe('(a ? b : c) + x');
      });
      ...
      it('wraps object expression inside binary', () => {
        ...
        expect(stringifyNode(node)).toBe('({ a: 1 }) + x');
      });
      ...
      it('wraps logical as conditional test', () => {
        ...
        expect(stringifyNode(node)).toBe('(a && b) ? x : y');
      });
      ...
      it('wraps binary as index-access object', () => {
        ...
        expect(stringifyNode(node)).toBe('(a + b)[0]');
      });
      ...
      it('does not wrap array expression as index-access object', () => {
        ...
        expect(stringifyNode(node)).toBe('[a, b, c][1]');
      });
      ...
      it('wraps all three conditional parts when they are complex', () => {
        ...
        expect(stringifyNode(node)).toBe('(a && b) ? (c + d) : (e || f)');
      });
    });
    ...
    describe('expression chaining', () => {
      ...
      it('handles member access on index result', () => {
        ...
        expect(stringifyNode(node)).toBe('arr[0].length');
      });
      ...
      it('handles index access with complex index expression', () => {
        ...
        expect(stringifyNode(node)).toBe('arr[i + 1]');
      });
      ...
      it('handles deeply chained member access', () => {
        ...
        expect(stringifyNode(node)).toBe('a.b.c.d');
      });
    });
    ...
    describe('string literal escaping', () => {
      it('escapes newlines', () => {
        ...
        expect(stringifyNode(node)).toBe('"hello\\nworld"');
      });

      it('escapes tabs', () => {
        const node: tinyest.Str = [N.stringLiteral, 'tab\there'];
        expect(stringifyNode(node)).toBe('"tab\\there"');
      });

      it('escapes backslashes', () => {
        const node: tinyest.Str = [N.stringLiteral, 'back\\slash'];
        expect(stringifyNode(node)).toBe('"back\\\\slash"');
      });
  ...
  describe('stringifyNode', () => {
    describe('let/const without initializer', () => {
      it('handles let without initializer', () => {
        ...
        expect(stringifyNode(node)).toBe('let x;');
      });

      it('handles const without initializer', () => {
        const node: tinyest.Const = [N.const, 'y'];
        expect(stringifyNode(node)).toBe('const y;');
      });
    });
    ...
    describe('for loop with missing parts', () => {
      it('handles infinite for loop (all parts null)', () => {
        ...
        expect(stringifyNode(node)).toMatchInlineSnapshot(`
          "for (; ; ) {
    ...
    describe('return with complex expressions', () => {
      it('handles return with nested binary expression', () => {
        ...
        expect(stringifyNode(node)).toBe('return a * (b + c);');
      });
      ...
      it('handles return with conditional expression', () => {
        ...
        expect(stringifyNode(node)).toBe('return flag ? 1 : 0;');
      });
    });
    ...
    describe('nested blocks', () => {
      it('handles nested block indentation', () => {
        ...
        expect(stringifyNode(outer)).toMatchInlineSnapshot(`
          "{
          {
              const x = 1;
            }
