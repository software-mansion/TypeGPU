...
describe('tgpu.immediateVar', () => {
  ...
  describe('compute pipelines', () => {
    ...
    it('ignores provided immediates the shader does not use', ({ root, computePassEncoder }) => {
      const writes = trackWrites(computePassEncoder.mock.setImmediates);
      const used = tgpu['~unstable'].immediateVar(d.f32);
      const unused = tgpu['~unstable'].immediateVar(d.f32);
      const entry = tgpu.computeFn({ workgroupSize: [1] })(() => {
        used.$;
      });
  ...
  describe('render pipelines', () => {
    it('rejects distinct immediates across stages of one shader module', ({ root }) => {
      const vertexValue = tgpu['~unstable'].immediateVar(d.f32, 0);
      const fragmentValue = tgpu['~unstable'].immediateVar(d.f32, 0);
      const pipeline = root
        .createRenderPipeline({
          vertex: () => {
            'use gpu';
            return { $position: d.vec4f(vertexValue.$) };
          },
          fragment: () => {
            'use gpu';
            return d.vec4f(fragmentValue.$);
          },
      ...
      expect(() => pipeline.draw(3)).toThrow(
        /WGSL allows at most one immediate variable per shader module/,
      );
    });
  ...
  describe('pass-level immediates', () => {
    ...
    it('prefers pass-level immediates over pipeline-level ones', ({ root, renderPassEncoder }) => {
      const writes = trackWrites(renderPassEncoder.mock.setImmediates);
      const pipeline = root.createRenderPipeline({ vertex: mainVertex, fragment: mainFragment });
    ...
    it('keeps pass-level immediates across pipeline switches', ({ root, renderPassEncoder }) => {
      const writes = trackWrites(renderPassEncoder.mock.setImmediates);
      const pipeline = root.createRenderPipeline({ vertex: mainVertex, fragment: mainFragment });
      ...
      expect(writes).toEqual([[0.25], [0.5]]);
    });
