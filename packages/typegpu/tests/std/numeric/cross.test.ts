...
describe('cross', () => {
  it('computes cross product of two vec3f', () => {
    ...
    expect(cross(vec3f(-1, 1, 0), vec3f(1, 0, 1))).toStrictEqual(
      vec3f(1, 1, -1),
    );
  });
  ...
  it('computes cross product of two vec3h', () => {
    ...
    expect(cross(vec3h(-1, 1, 0), vec3h(1, 0, 1))).toStrictEqual(
      vec3h(1, 1, -1),
    );
    expect(cross(vec3h(2, 2, 0), vec3h(1, 4, 1))).toStrictEqual(
      vec3h(2, -2, 6),
    );
  });
});
