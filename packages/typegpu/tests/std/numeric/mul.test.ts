...
import type { m2x2f, m3x3f, m4x4f, v2f, v3f, v4f } from 'typegpu/data';
...
describe('mul overload', () => {
  ...
  it('rejects when incompatible types', () => {
    ...
    expect(() => mul(true, false)).toThrowErrorMatchingInlineSnapshot(
      `[Error: Unsupported signature. Expected one of 'number, vec2i, vec3i, vec4i, vec2f, vec3f, vec4f, vec2h, vec3h, vec4h, vec2, vec3, vec4, vec2u, vec3u, vec4u, mat2x2f, mat3x3f, mat4x4f', got 'boolean'.]`,
    );
  });
});
