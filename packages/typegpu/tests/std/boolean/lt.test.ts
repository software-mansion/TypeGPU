...
describe('lt', () => {
  it('compares integer vectors', () => {
    ...
    expect(lt(vec3i(10, 20, 20), vec3i(10, 20, 30))).toStrictEqual(
  ...
  it('compares float vectors', () => {
    expect(lt(vec2f(0.1, 1.1), vec2f(0.1, 2))).toStrictEqual(
      vec2b(false, true),
    );
    expect(lt(vec3f(1.2, 2.3, 3.4), vec3f(2.3, 3.2, 3.4))).toStrictEqual(
      vec3b(true, true, false),
    );
    expect(
      lt(vec4f(0.1, -0.2, -0.3, 0.4), vec4f(0.1, 0.2, 0.3, 0.4)),
