...
describe('select', () => {
  ...
  it('throws on invalid arguments', () => {
    ...
    expect(() => select(mat2x2f(), mat2x2f(), true)).toThrowErrorMatchingInlineSnapshot(
      `[Error: Unsupported signature. Expected one of 'number, vec2i, vec3i, vec4i, vec2f, vec3f, vec4f, vec2h, vec3h, vec4h, vec2, vec3, vec4, vec2u, vec3u, vec4u, boolean, vec2<bool>, vec3<bool>, vec4<bool>', got 'mat2x2f'.]`,
    );
    // @ts-expect-error
