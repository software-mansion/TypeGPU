...
import { isWgslStorageTexture, isWgslTexture } from '../data/texture.ts';
...
export type SerializedDataSchema =
  ...
  | { type: 'sampled-texture'; kind: d.WgslTexture['type']; sampleType: SerializedDataSchema }
  | {
...
export function deserializeDataSchema(schema: SerializedDataSchema): d.AnyData {
  ...
  if (schema.type === 'sampled-texture') {
    ...
    return constructor(
      deserializeDataSchema(schema.sampleType) as d.WgslTexture['sampleType'],
    ) as d.AnyData;
  }

  if (schema.type === 'external-texture') {
    return d.textureExternal() as d.AnyData;
  }
  ...
  if (schema.type === 'storage-texture') {
    ...
    if (!constructor) {
      throw new Error(
        `TypeGPU storage texture schema '${schema.kind}' could not be reconstructed.`,
      );
    }
    return constructor(schema.format, schema.access) as d.AnyData;
  }
