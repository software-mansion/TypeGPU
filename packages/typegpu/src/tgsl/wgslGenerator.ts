...
export class WgslGenerator implements ShaderGenerator {
  ...
  public initGenerator(ctx: ResolutionCtx) {
    ...
    this.#ctx = ctx;
    this._reserveGlobals();
  }

  /**
   * Reserves all builtin WGSL globals. Can be overriden to change behavior.
   */
  protected _reserveGlobals() {
    for (const ident of builtinGlobals) {
      this.ctx.reserveIdentifier(ident, 'global');
    }
  }
  ...
  public isBannedToken(token: string): boolean {
    return bannedTokens.has(token);
  }

  public isBuiltinGlobal(identifier: string): boolean {
