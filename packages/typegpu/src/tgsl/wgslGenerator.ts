    ...
    if (
      ...
      if (exprType === NODE.assignmentExpr) {
        if (
          ...
          throw new WgslTypeError(
            `'${lhsStr} = ${rhsStr}' is invalid, because ${lhsStr} is a constant. This error may also occur when accidentally assigning to a value defined outside of a TypeGPU function.`,
          );
        }
