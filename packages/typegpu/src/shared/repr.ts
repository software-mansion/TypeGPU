 ...
 */
export type InferGPU<T> = T extends { readonly [$gpuRepr]: infer TRepr } ? TRepr : Infer<T>;
...
export type IsValidStorageSchema<T> = (
  T extends { readonly [$validStorageSchema]: true } ? true : false
) extends false //
  ? false
  : true;
...
export type IsValidUniformSchema<T> = (
  T extends { readonly [$validUniformSchema]: true } ? true : false
) extends false //
  ? false
  : true;
...
export type IsValidVertexSchema<T> = (
  T extends { readonly [$validVertexSchema]: true } ? true : false
) extends false //
  ? false
  : true;
 ...
 */
export type IsValidIndexSchema<T> =
  Undecorate<T> extends WgslArray | Disarray
    ? Undecorate<Undecorate<T>['elementType']> extends U32 | U16
      ? true
      : false
    : false;
...
export type IsValidBufferSchema<T> =
  ...
  | IsValidIndexSchema<T> extends false
  ? false
  : true;
...
export type IsValidSampledTextureUsage<TTexture extends TgpuTexture> =
  TTexture['usableAsSampled'] extends true
    ? true
    : {
...
export type IsValidStorageTextureUsage<TTexture extends TgpuTexture> =
  TTexture['usableAsStorage'] extends true
    ? true
    : {
...
> = ViewDimensionToDimension[TSchema['dimension']] extends infer TVDim
  ? TVDim extends Default<TTexture['props']['dimension'], '2d'>
    ? true
    : {
...
> = TSchema['format'] extends TTexture['props']['format']
  ? true
  : TTexture['props']['viewFormats'] extends readonly unknown[]
    ? TSchema['format'] extends TTexture['props']['viewFormats'][number]
      ? true
...
type SelfOrErrors<TSelf, T> =
  IsExactly<T, true> extends true
    ? TSelf
...
> = IsValidSampledTextureUsage<TTexture> & IsValidSubdimension<TTexture, TSchema>;
...
export type ExtractInvalidSchemaError<T, TPrefix extends string = ''> = [T] extends [
