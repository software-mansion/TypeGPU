...
export const subgroupBroadcast = dualImpl<IdentityNumOrVecWithIdx>({
  ...
  signature: (...args) => {
    ...
    if (!id) {
      throw new Error(
        `subgroupBroadcast's second argument has to be compatible with i32 or u32. Got: ${String(
          args[1],
        )}`,
      );
    }
...
export const subgroupShuffle = dualImpl<IdentityNumOrVecWithIdx>({
  ...
  signature: (...args) => {
    ...
    if (!id) {
      throw new Error(
        `subgroupShuffle's second argument has to be compatible with i32 or u32. Got: ${String(
          args[1],
        )}`,
      );
    }
...
export const subgroupShuffleDown = dualImpl<IdentityNumOrVecWithDelta>({
  ...
  signature: (...args) => {
    ...
    if (!delta) {
      throw new Error(
        `subgroupShuffleDown's second argument has to be compatible with u32. Got: ${String(args[1])}`,
      );
    }
...
export const subgroupShuffleUp = dualImpl<IdentityNumOrVecWithDelta>({
  ...
  signature: (...args) => {
    ...
    if (!delta) {
      throw new Error(
        `subgroupShuffleUp's second argument has to be compatible with u32. Got: ${String(args[1])}`,
      );
    }
    return { argTypes: [args[0], delta[0]], returnType: args[0] };
  },
...
export const subgroupShuffleXor = dualImpl<IdentityNumOrVecWithMask>({
  ...
  signature: (...args) => {
    ...
    if (!mask) {
      throw new Error(
        `subgroupShuffleXor's second argument has to be compatible with u32. Got: ${String(args[1])}`,
      );
    }
