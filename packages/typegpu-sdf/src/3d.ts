 ...
 */
export const sdSphere = tgpu.fn([vec3f, f32], f32)((p, r) => {
  'kernel & js';
  return length(p) - r;
});

export const sdBox = tgpu.fn([vec3f, vec3f], f32)((p, b) => {
  'kernel & js';
  const d = sub(abs(p), b);
  return length(max(d, vec3f(0))) + min(max(max(d.x, d.y), d.z), 0);
});

export const sdRoundedBox = tgpu.fn([vec3f, vec3f, f32], f32)((p, b, r) => {
  'kernel & js';
 ...
 */
export const sdBoxFrame = tgpu.fn([vec3f, vec3f, f32], f32)((p, b, e) => {
  'kernel & js';
