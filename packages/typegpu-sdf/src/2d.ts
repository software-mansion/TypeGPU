 ...
 */
export const sdRoundedBox2d = tgpu.fn(
  [vec2f, vec2f, f32],
  f32,
...
export const sdBezier = tgpu.fn(
  ...
  if (h >= 0.0) {
    ...
    res = min(dot2(d.add(c.add(b.mul(t.x)).mul(t.x))), dot2(d.add(c.add(b.mul(t.y)).mul(t.y))));
  }
