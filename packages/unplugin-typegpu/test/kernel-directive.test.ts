...
describe('[BABEL] "kernel" directive', () => {
  it('makes plugin transpile marked arrow functions', () => {
    ...
    expect(babelTransform(code)).toMatchInlineSnapshot(`
      "import tgpu from 'typegpu';
      const addGPU = tgpu.__assignAst(tgpu.__removedJsImpl("addGPU"), {"argNames":{"type":"identifiers","names":["a","b"]},"body":[3,[[1,[10,"a","+","b"]]]],"externalNames":[]}, {});
      const addCPU = (a, b) => {
        return a + b;
      };"
    `);
  ...
  it('makes plugin transpile marked arrow functions passed to shells', () => {
    ...
    expect(babelTransform(code)).toMatchInlineSnapshot(`
      "import tgpu from 'typegpu';
      const shell = tgpu['~unstable'].fn([]);
      shell(tgpu.__assignAst(tgpu.__removedJsImpl(), {"argNames":{"type":"identifiers","names":["a","b"]},"body":[3,[[1,[10,"a","+","b"]]]],"externalNames":[]}, {}));
      shell((a, b) => {
        return a + b;
      });"
    `);
  ...
  it('makes plugin transpile marked non-arrow named functions passed to shells', () => {
    ...
    expect(babelTransform(code)).toMatchInlineSnapshot(`
      "import tgpu from 'typegpu';
      const shell = tgpu['~unstable'].fn([]);
      shell(tgpu.__assignAst(tgpu.__removedJsImpl("addGPU"), {"argNames":{"type":"identifiers","names":["a","b"]},"body":[3,[[1,[10,"a","+","b"]]]],"externalNames":[]}, {}));
      shell(function addCPU(a, b) {
        return a + b;
      });"
    `);
