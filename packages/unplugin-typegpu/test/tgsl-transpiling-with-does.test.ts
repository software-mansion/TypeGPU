...
describe('[ROLLUP] plugin for transpiling tgsl functions to tinyest', () => {
  ...
  it('works for multiple functions, skips wgsl-implemented', async () => {
    ...
    expect(await rollupTransform(code)).toMatchInlineSnapshot(`
      ...
      const a = $autoName(tgpu['~unstable'].computeFn({ workgroupSize: [1] }).does((($) => ((globalThis.__TYPEGPU_META__ ??= new WeakMap()).set(
                      $.f = (() => {
                        throw new Error(\`The function "<unnamed>" is invokable only on the GPU. If you want to use it on the CPU, mark it with the "kernel & js" directive.\`);
                      }) , {
                    v: 1,
              ...
              const b = $autoName(tgpu['~unstable'].fn([]).does((($) => ((globalThis.__TYPEGPU_META__ ??= new WeakMap()).set(
                      $.f = (() => {
                        throw new Error(\`The function "<unnamed>" is invokable only on the GPU. If you want to use it on the CPU, mark it with the "kernel & js" directive.\`);
                      }) , {
                    v: 1,
              ...
              const c = $autoName(tgpu['~unstable'].fn([]).does((($) => ((globalThis.__TYPEGPU_META__ ??= new WeakMap()).set(
                      $.f = (() => {
                        throw new Error(\`The function "<unnamed>" is invokable only on the GPU. If you want to use it on the CPU, mark it with the "kernel & js" directive.\`);
                      }) , {
                    v: 1,
              ...
              const d = $autoName(tgpu['~unstable'].fn([]).does('() {}'), d);
