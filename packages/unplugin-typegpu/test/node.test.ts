...
describe('Node.js and Deno loader hooks', () => {
  ...
  it('matches remote modules by their decoded URL', () => {
    const result = { source: shader, format: 'module' };
    const load = loadHook({ include: '**/my shaders/*.ts' });
    expect(
      load('https://example.com/my%20shaders/test.ts?v=1', context, () => result).source,
    ).toContain('__TYPEGPU_META__');
    const originLoad = loadHook({ include: /^https:\/\/example\.com\// });
    expect(originLoad('https://example.com/test.ts', context, () => result).source).toContain(
      '__TYPEGPU_META__',
    );
    expect(originLoad('https://other.com/test.ts', context, () => result)).toBe(result);
  });

  it('inlines a source map pointing to the original source', () => {
    const transformed = loadHook()('file:///shaders/my%20shader.ts', context, () => ({
      source: shader,
      format: 'module',
    }));
    const match = String(transformed.source).match(
      /\n\/\/# sourceMappingURL=data:application\/json;charset=utf-8;base64,(.+)$/,
    );
    expect(match).not.toBeNull();
    const map = JSON.parse(Buffer.from(match?.[1] ?? '', 'base64').toString());
    expect(map.sources).toEqual(['/shaders/my shader.ts']);
    expect(map.sourcesContent).toEqual([shader]);
  });
