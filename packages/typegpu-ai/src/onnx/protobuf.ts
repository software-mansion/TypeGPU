...
export class PbReader {
  ...
  bytes(): Uint8Array {
    ...
    const end = start + len;
    if (end > this.buf.length) throw new Error('Truncated length-delimited field');
    this.pos = end;
  ...
  skip(wt: WireType): void {
    switch (wt) {
      case WireType.Varint: this.varint(); return;
      case WireType.Bit64: this.seek(this.pos + 8); return;
      case WireType.LengthDelimited: { const len = this.varint(); this.seek(this.pos + len); return; }
      case WireType.Bit32: this.seek(this.pos + 4); return;
