...
import { type ScanSchemas, WORKGROUP_SIZE } from '../schemas.ts';

export function makeShared(schemas: ScanSchemas) {
  const { operatorSlot, workgroupMemory } = schemas;

  const upsweep = tgpu.fn([d.u32])((localIdx) => {
    let offset = d.u32(1);
  ...
  const downsweep = tgpu.fn([d.u32])((localIdx) => {
