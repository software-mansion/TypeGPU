...
export class GlslGenerator extends WgslGenerator {
  ...
  #reservedIdentifiers: boolean = false;
  #hoistedIndexCount = 0;
  ...
  override initGenerator(ctx: ResolutionCtx) {
    ...
    this.#reservedIdentifiers = false;
    this.#hoistedIndexCount = 0;
  ...
  override typeInstantiation(schema: d.BaseData, args: Snippet[]): ResolvedSnippet {
    ...
    if (schema.type === 'array') {
      ...
      return snip(
        `${element}[${arraySchema.elementCount}](${resolvedArgs})`,
        superSnippet.dataType,
        superSnippet.origin,
        superSnippet.possibleSideEffects,
      );
    }
  ...
  override functionDefinition(options: FunctionDefinitionOptions): string {
    if (!this.#reservedIdentifiers) {
      this.#reservedIdentifiers = true;
